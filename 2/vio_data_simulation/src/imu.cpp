...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        MotionData imu1 = imudata[i-1];   //上一个时刻的IMU信息

        //
        //delta_q = [1 , 1/2 * thetax , 1/2 * theta_y, 1/2 * theta_z]
        Eigen::Quaterniond dq;
        Eigen::Vector3d dtheta_half =  0.5*(imupose.imu_gyro+imu1.imu_gyro) * dt /2.0;      //w = 1/2(w1+w2)
        dq.w() = 1;
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        dq.z() = dtheta_half.z();
        ...
        /// 中值积分
        Eigen::Quaterniond Qwb1 = Qwb * dq;   //Qwb1当前时刻的姿态四元数，Qwb为上一时刻的
        Eigen::Vector3d acc_w =  0.5*(Qwb1*imupose.imu_acc+Qwb*imu1.imu_acc)+gw; //中值法的公式，这里gw取符号选取根定义有关
