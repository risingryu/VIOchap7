...
void Problem::ComputeLambdaInitLM() {
    ...
    for (auto edge: edges_) {
        currentChi_ += edge.second->Chi2();     //残差相加
    }
...
void Problem::AddLambdatoHessianLM() {
    ...
    assert(Hessian_.rows() == Hessian_.cols() && "Hessian is not square");
    for (ulong i = 0; i < size; ++i) {
        //Hessian_(i, i) += currentLambda_;     //原来的方法
...
//第一种策略：
 ...
 // 统计所有的残差
    ...
    MatXX diag_JWJ(MatXX::Zero(size, size));
    for (ulong i = 0; i < size; ++i) {
        diag_JWJ(i, i) = Hessian_(i, i);
    }
    double scale = delta_x_.transpose() * (currentLambda_ * diag_JWJ * delta_x_ + b_);
...
//     double chi_ =   tempChi-currentChi_;
//     double alpha_fenzi = b_.transpose()*delta_x_;//  //第二种方法的delta x
//     double alpha_fenmu =chi_/2+2*b_.transpose()*delta_x_;    //shishi fu hao 
//     double alpha = alpha_fenzi/alpha_fenmu+1e-1;

//     RollbackStates();   // 迭代方式改变了，回滚
//     delta_x_ = alpha*delta_x_;
//     UpdateStates();   //fengxin
...
//      double rho = (currentChi_ - tempChi) / scale; //新的rho
