...
int main()
{
    ...
    for(int j = 0; j < featureNums; ++j)
    {
        ...
        for (int i = 0; i < poseNums; ++i) {
            ...
            /// 请补充完整作业信息矩阵块的计算
	    // H.block(?,?,?,?) += ?;
         H.block(poseNums*6+j*3,poseNums*6+j*3,3,3) +=  jacobian_Pj.transpose()* jacobian_Pj;       //  Jacobian 的jj项
         
	    // // H.block(?,?,?,?) += ?;
         H.block(i*6,poseNums*6+j*3,6,3) +=  jacobian_Ti.transpose() *jacobian_Pj;     //  Jacobian 的ij项
        //     // H.block(?,?,?,?) += ?;
         H.block(poseNums*6+j*3,i*6,3,6) +=  jacobian_Pj.transpose()*  jacobian_Ti;   //  Jacobian 的ji项
