...
int main() {
    ...
    GetSimDataInWordFrame(cameras, points);    //&变量名是引用可以修改值
    ...
    // 所有 Pose
    vector<shared_ptr<VertexPose> > vertexCams_vec;     //存放所有的相机位姿
    for (size_t i = 0; i < cameras.size(); ++i) {
        ...
        vertexCam->SetParameters(pose);//数据的顺序，T，虚部的XYZ实部的W

    //    if(i < 2)
    //        vertexCam->SetFixed();      //固定前两个相机顶点，具体操作时雅可比为0

        problem.AddVertex(vertexCam);   //  添加相机顶点
...
//        double inverse_depth = 1. / Pc.z();
        ...
        problem.AddVertex(verterxPoint);    //添加路标点的顶点，只添加进了逆深度？这个程序只优化逆深度？那landmark只有一维？
        allPoints.push_back(verterxPoint);

       //添加先验信息
        double prior_factor = 1e15;
     for (size_t k = 0; k < 2; ++k) {
         shared_ptr<EdgeSE3Prior> prior_info(new EdgeSE3Prior(cameras[k].twc, cameras[k].qwc));
          std::vector<std::shared_ptr<Vertex> > prior_info_vertex;
            prior_info_vertex.push_back(vertexCams_vec[k]);
           prior_info->SetVertex(prior_info_vertex);
          prior_info->SetInformation(prior_info->Information() * prior_factor);
           problem.AddEdge(prior_info);
        }


        // 每个特征对应的投影误差, 第 0 帧为起始帧
        for (size_t j = 1; j < cameras.size(); ++j) {
            Eigen::Vector3d pt_i = cameras[0].featurePerId.find(i)->second;   //返回归一化平面的相机坐标
