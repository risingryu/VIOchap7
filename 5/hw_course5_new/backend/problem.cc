...
void Problem::AddOrderingSLAM(std::shared_ptr<myslam::backend::Vertex> v) {
    if (IsPoseVertex(v)) {
        ...
        ordering_poses_ += v->LocalDimension();     //这一句表示有pose的id有纬度的信息
    ...
    for (auto &edge: edges_) {
        ...
        for (size_t i = 0; i < verticies.size(); ++i) {
            ...
            for (size_t j = i; j < verticies.size(); ++j) {
                ...
                if(i==j&&i<=1)   //给第一二帧增加先验
                {
                    double inf_factor=10e15;
                    MatXX prior_;
                    H.block(index_i,index_j, dim_i, dim_j).noalias() += inf_factor*prior_.Identity(dim_i,dim_j);
                }
...
//        delta_x_ = PCGSolver(H, b_, H.rows() * 2);
        ...
        // VecX bpp = b_.segment(?,?);
        // VecX bmm = b_.segment(?,?);
        MatXX Hmm = Hessian_.block(reserve_size,reserve_size, marg_size, marg_size);   //pose   block
...
void Problem::ComputeLambdaInitLM() {
    ...
    for (ulong i = 0; i < size; ++i) {
        maxDiagonal = std::max(fabs(Hessian_(i, i)), maxDiagonal);  //fabs求绝对值
    }
    double tau = 1e-5;
